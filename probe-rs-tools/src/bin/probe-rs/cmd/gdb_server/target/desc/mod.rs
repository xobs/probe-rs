...
impl RuntimeTarget<'_> {
    pub(crate) fn load_target_desc(&mut self) -> Result<(), Error> {
        ...
        let mut core = session.core(self.cores[0]).unwrap();

        self.target_desc =
            build_target_description(core.registers(), core.core_type(), core.instruction_set().expect("breakage"));
...
impl MemoryMap for RuntimeTarget<'_> {
    fn memory_map_xml(
        ...
        let mut session = self.session.lock();
        let xml = gdb_memory_map(&mut session, self.cores[0]).unwrap();
        let xml_data = xml.as_bytes();
...
fn gdb_memory_map(session: &mut Session, primary_core_id: usize) -> Result<String, Error> {
    let (virtual_addressing, address_size) = {
        let core = session.core(primary_core_id).unwrap();
